...
def _cstats(coords, d, tol=fe.DOLFIN_EPS):
    ...
    xtol = tol*(xmax - xmin)
    if abs(xmin) > xtol:
        logPERIODIC('xmin, xtol', xmin, xtol)
        raise KSDGException(
    ...
    if np.max(np.abs(dxs - dx)) > xtol:
        logPERIODIC('tol,xtol', tol, xtol)
        logPERIODIC('dxs', dxs)
        logPERIODIC('np.max(np.abs(dxs - dx))', np.max(np.abs(dxs - dx)))
        raise KSDGException(
...
class CornerDomain(SubDomain):
    def __init__(
        ...
        logPERIODIC('CornerDomain __init__')
        super().__init__(map_tol=tol)
        ...
        if (xmin is None and xmid is None):
            ...
            self.xmin = xmin
            self.xmid = xmid
        logPERIODIC('self.xmin, self.xmid', self.xmin, self.xmid)
        self.lb = self.xmin - 2*self.tol*(self.xmid - self.xmin)
        self.ub = self.xmid + 2*self.tol*(self.xmid - self.xmin)
        logPERIODIC('self.lb, self.ub', self.lb, self.ub)
        logPERIODIC('CornerDomain __init__ returning')
